...
typedef struct{
  ...
  void menu(){
	...
	printf("                             Benrabiaa Ayman / Lekkioui Anasse / Mazouz Achraf    \n\n");

	printf("  _______________________________________________Menu_____________________________________________________\n ");
	printf("|                                                                                                        |\n ");
    ...
    scanf("%d",&cin);
       for(j=0;j<*nbc;j++){
        ...
        scanf("%d",&mat);
         ...
         if(j==*nbv)     printf("ERREUR: voiture introuvable!\n");
        printf("Date de location:\n");
        printf("jours./");
...
void pgrand_chiffre(voiture *v,int *y){
  ...
  int mat;
  for(i=0;i<*y;i++){
    if((v+i)->chiffre>(v+i+1)->chiffre)      mat=(v+i)->matricule;
  }
  for(i=0;i<*y;i++){
    if((v+i)->matricule==mat){
...
while (chx!=17){
 ...
 switch (chx){
    ...
    case 2:
   		...
   		ajout_client(c,nbc);clrscr(); clrscr();
		   break;
 	case 3:
    	nbv++;
    	v=(voiture *)realloc(v,nv*sizeof(voiture));
   		ajout_voiture(v,nbv);clrscr();clrscr();clrscr();
		   break;
    case 4:
        nbl++;
   		l=(location *)realloc(l,nl*sizeof(location));
        ajout_location(c,v,l,nbc,nbv,nbl);clrscr();
		break;
    case 5:
    	aff_voiture(v,nbv);clrscr();
		break;
    case 6:
    	aff_client(c,nbc);clrscr();
		break;
    case 7:
    	aff_location(c,v,l,nbc,nbv,nbl);clrscr();
		break;
  	case 8:
    	affpltr(c,nbc);clrscr();
		break;
    case 9:
    	supp_client(c,nbc);clrscr();
		break;
    case 10:
    	supp_voiture(v,nbv);clrscr();
		break;
    case 11:
    	supp_client(c,nbc);clrscr();
		break;
    case 12:
    	n=cpt_contact(c,nbc);
     	printf("nombre de contact de cette ville : %d",n); clrscr();
        break;
    case 13:
     	chiff_aff(v,nbv);clrscr();
		 break;
    case 14:
        pgrand_chiffre(v,nbv);clrscr();
		break;
  	case 15:
     	init_tab_cin(c,l,l2,nbc,nbl,nbl2);clrscr();
		 break;
  	case 16:
     	init_tab_mat(v,l,l3,nbv,nbl,nbl3);clrscr();
		break;
        default:
        break;
    }
